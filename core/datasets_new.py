...
class VideoColorizationDataset(Dataset):
    ...
    def _load_and_process_image(self, path):
        ...
        try:
            ...
            # ===== 2. 處理彩色 RGB（給 cnet） =====
            rgb_color = self.to_tensor(image)         # [3, H, W]
    ...
    def _apply_augmentation(self, rgb_list, rgb_color_list, lab_list):
        ...
        if random.random() > 0.5:
            ...
            rgb_color_list = [torch.flip(rgb, [-1]) for rgb in rgb_color_list]
    ...
    def __getitem__(self, idx):
        ...
        for frame_path in seq_info['frame_paths']:
            rgb_gray, rgb_color, lab = self._load_and_process_image(frame_path)
        ...
        rgb_color_seq = torch.stack(rgb_color_list, dim=0)  # [4, 3, H, W]
        lab_seq = torch.stack(lab_list, dim=0)        # [4, 3, H, W]

        # ===== 準備輸出 (MemFlowNet格式) =====
        return {
            'images': lab_seq,           # [4, 3, H, W] - LAB序列
            'rgb_inputs': rgb_seq,       # [4, 3, H, W] - 灰階RGB序列（給 fnet）
            'rgb_color_inputs': rgb_color_seq,  # [4, 3, H, W] - 彩色RGB序列（給 cnet）
